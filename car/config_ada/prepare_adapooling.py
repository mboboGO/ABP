...
parser.add_argument('--resize', nargs='+', type=int, default=[448,480], help='The input size of the different multi-scale branches.')
...
lyr[0].image_data_param.new_height = 448
lyr[0].image_data_param.new_width = 448

lyr[1].image_data_param.new_height = 448
lyr[1].image_data_param.new_width = 448
...
if(1):
    ...
    netparams.layer[-1].reshape_param.shape.dim.append(0)
    netparams.layer[-1].reshape_param.shape.dim.append(0)
    netparams.layer[-1].reshape_param.shape.dim.append(crop_size/16)
    netparams.layer[-1].reshape_param.shape.dim.append(crop_size/16)
...
params.base_lr = 0.1
params.display = 100
params.max_iter = 140 * iter_per_epoch
params.lr_policy = "step"
params.gamma = 0.25
...
solver.net.copy_from('../config_cbp/model_init')
